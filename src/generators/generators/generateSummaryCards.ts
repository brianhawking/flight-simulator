...
export function generateSummaryCards(scenario: ScenarioModel): EvidencePackage["summaryCards"] {
  ...
  if (hasMetricSignal(scenario, "activation_success_rate")) {
    return generateBusinessSummaryCards(scenario);
  }

  if (hasMetricSignal(scenario, "feature_tile_success_rate")) {
    return generateClientSummaryCards(scenario);
  }

  throw new Error(`Unsupported signal combination for summary card generation: ${scenario.id}`);
}
...
function generateClientSummaryCards(scenario: ScenarioModel): EvidencePackage["summaryCards"] {
...
function generateBusinessSummaryCards(scenario: ScenarioModel): EvidencePackage["summaryCards"] {
  const apiHealthSignal = getRequiredMetricSignal(scenario, "api_success_rate");
...
function hasMetricSignal(scenario: ScenarioModel, metric: string) {
  return scenario.signalParameters.some(
